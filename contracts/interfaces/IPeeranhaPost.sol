...
interface IPeeranhaPost {
    ...
    function publicationReply(address name, uint32 postId, bool officialReply, uint16[] memory path, bytes32 ipfsHash) external;
    function publicationComment(address name, uint32 postId, uint16[] memory path, bytes32 ipfsHash) external;
