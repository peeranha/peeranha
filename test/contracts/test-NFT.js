...
describe("Test NFT", function () {
	...
	describe("Test common NFT", function () {
		...
		it("Add achievement with 0 token pool", async function () {
			...
			const peeranhaAchievement = await peeranhaUser.getAchievementConfig(1);
			await expect(await getInt(peeranhaAchievement.maxCount)).to.equal(0);
			const peeranhaAchievementNFT = await peeranhaNFT.getAchievementsNFTConfig(1);
			await expect(await getInt(peeranhaAchievementNFT.maxCount)).to.equal(0);
		});
		...
		it("New admin add achievement", async function () {
			...
			peeranhaUser.connect(signers[1]).configureNewAchievement(100, 15, URIContainer[0], 0, AchievementsType.Rating);
		});
		...
		it("Test give 1st NFT", async function () {
			...
			const tokenId = await peeranhaNFT.tokenOfOwnerByIndex(accountDeployed, 0); 
		...
		it("Test add achievement after user achieved its requirements", async function () {
			...
			await peeranhaUser.addUserRating(accountDeployed, 1, 1);
			const tokensCountAfter = await peeranhaNFT.balanceOf(accountDeployed);
			await expect(await getInt(tokensCountAfter)).to.equal(1);
			const tokenIdAfter = await peeranhaNFT.tokenOfOwnerByIndex(accountDeployed, 0);
		...
		it("Test give 1st NFT (try double)", async function () {
			...
			await peeranhaUser.addUserRating(accountDeployed, 5, 1);
			await peeranhaUser.addUserRating(accountDeployed, -1, 1);
			await peeranhaUser.addUserRating(accountDeployed, 1, 1);
		...
		it("Test give NFT 2 users", async function () {
			...
			const tokensCountFirstUser = await peeranhaNFT.balanceOf(accountDeployed);
			await expect(await getInt(tokensCountFirstUser)).to.equal(1);
			const tokenIdFirstUser = await peeranhaNFT.tokenOfOwnerByIndex(accountDeployed, 0);
		...
		it("Test give NFTs for 2 achievements", async function () {
			...
			await peeranhaUser.configureNewAchievement(5, 15, URIContainer[0], 0, AchievementsType.Rating);
			await peeranhaUser.configureNewAchievement(5, 20, URIContainer[1], 0, AchievementsType.Rating);

			await peeranhaUser.addUserRating(accountDeployed, 5, 1);

			const tokensCount1 = await peeranhaNFT.balanceOf(accountDeployed);
			...
			const tokenId = await peeranhaNFT.tokenOfOwnerByIndex(accountDeployed, 0);
			await expect(await getInt(tokenId)).to.equal(1);
			const tokenId2 = await peeranhaNFT.tokenOfOwnerByIndex(accountDeployed, 1);
		...
		it("Test transfer token", async function () {
			...
			await peeranhaNFT.transferFrom(accountDeployed, signers[1].address, 1);

			const tokensCountFirstUser2 = await peeranhaNFT.balanceOf(accountDeployed);
	...
	describe("Test soul bound NFT", function () {
		...
		it("Test add soul bound achievement (10 max count)", async function () {
			...
			await expect(await getInt(peeranhaAchievement.factCount)).to.equal(0);
			await expect(await getInt(peeranhaAchievement.lowerBound)).to.equal(0);
			await expect(peeranhaAchievement.achievementsType).to.equal(2);
	
			const peeranhaAchievementNFT = await peeranhaNFT.getAchievementsNFTConfig(1)
			await expect(await getInt(peeranhaAchievementNFT.maxCount)).to.equal(10);
			await expect(await getInt(peeranhaAchievementNFT.factCount)).to.equal(0);
			await expect(peeranhaAchievementNFT.achievementURI).to.equal(URIContainer[0]);
			await expect(peeranhaAchievementNFT.achievementsType).to.equal(2);
		});
		...
		it("Test give 1st NFT achiuevement's community - 0 (soul rating)", async function () {
			...
			await expect(communityId).to.equal(0);
			await peeranhaUser.addUserRating(accountDeployed, 10, 1);
		...
		it("Test give 1st NFT  the same achiuevement's community (soul rating)", async function () {
			...
			const communityId = await peeranhaUser.getAchievementCommunity(1);
			await expect(communityId).to.equal(1);
			await peeranhaUser.addUserRating(accountDeployed, 10, 1);
		...
		it("Test give 1st NFT another achiuevement's community  (soul rating)", async function () {
			...
			const communityId = await peeranhaUser.getAchievementCommunity(1);
			await expect(communityId).to.equal(2);
			await peeranhaUser.addUserRating(accountDeployed, 10, 1);
		...
		it("Test give rating and soulRating NFT ", async function () {
			...
			const tokensCount = await peeranhaNFT.balanceOf(accountDeployed);
			await expect(await getInt(tokensCount)).to.equal(2);
			...
			const tokenURI = await peeranhaNFT.tokenURI(tokenId);
			await expect(tokenURI).to.equal(URIContainer[0]);
			const tokenURI2 = await peeranhaNFT.tokenURI(tokenId2);
			await expect(tokenURI2).to.equal(URIContainer[1]);
		});
		...
		it("Test transefer soulRating NFT, rating nft exist (soul bound first)", async function () {
			...
			await expect(peeranhaNFT.transferFrom(accountDeployed, signers[1].address, 1))
				.to.be.revertedWith('You_can_not_transfer_soul_bound');
		...
		it("Test transefer soulRating NFT, rating nft exist (soul bound second)", async function () {
			...
			await peeranhaNFT.transferFrom(accountDeployed, signers[1].address, 1);
			await expect(peeranhaNFT.transferFrom(accountDeployed, signers[1].address, 1000001))
				.to.be.revertedWith('You_can_not_transfer_soul_bound');
		});
