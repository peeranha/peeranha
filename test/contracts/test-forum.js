...
describe("Test post", function () {
	...
	let signers, ipfsHashes, hashContainer;
	beforeEach(async function () {
		...
		await peeranhaUser.connect(signers[5]).createUser(signers[5].address, hashContainer[0]);
		...
		await peeranhaUser.grantRole(VERIFIED_ROLE, signers[5].address);
	...
	describe('Create post', function () {
		...
		xit("Test create post by not registered user", async function () { // create user createIfDoesNotExist
			await expect(peeranhaContent.connect(signers[10]).createPost(signers[10].address, 1, hashContainer[0], PostTypeEnum.ExpertPost, [1], LanguagesEnum.English))
				.to.be.revertedWith('user_not_found');
		});

		it("Test create post for non-existing community", async function () {
			await expect(peeranhaContent.createPost(signers[0].address, 10, hashContainer[0], PostTypeEnum.ExpertPost, [1], LanguagesEnum.English)).to.be.revertedWith('Community does not exist');
		});
	...
	describe('Create reply', function () {
		...
		xit("Test create reply by not registered user", async function () { // create user createIfDoesNotExist
			...
			await expect(peeranhaContent.connect(signers[10]).createReply(signers[10].address, 1, 0, hashContainer[0], false, LanguagesEnum.English))
			.to.be.revertedWith('user_not_found');
		});
	...
	describe('Create comment', function () {
		...
		xit("Test create comment by not registered user", async function () {  // create user createIfDoesNotExist
			...
			await expect(peeranhaContent.connect(signers[10]).createComment(signers[10].address, 1, 0, hashContainer[2]))
			.to.be.revertedWith('user_not_found');
			await expect(peeranhaContent.connect(signers[10]).createComment(signers[10].address, 1, 1, hashContainer[2]))
			.to.be.revertedWith('user_not_found');
		});
	...
	describe('Edit reply', function () {
		...
		it("Test edit reply by not registered user", async function () {
			...
			await expect(peeranhaContent.connect(signers[10]).editReply(signers[10].address, 1, 1, hashContainer[2], false, LanguagesEnum.English))
			.to.be.revertedWith('user_not_found');
		});
	...
	describe('Edit comment', function () {
		...
		it("Test edit comment by not registered user", async function () {
			...
			await expect(peeranhaContent.connect(signers[10]).editComment(signers[10].address, 1, 0, 1, hashContainer[2], LanguagesEnum.English))
			.to.be.revertedWith('user_not_found');
		});
	...
	describe('Delete post', function () {
		...
		it("Test delete post by not registered user", async function () {
			...
			await expect(peeranhaContent.connect(signers[10]).deletePost(signers[10].address, 1))
			.to.be.revertedWith('user_not_found');
		});
	...
	describe('Delete reply', function () {
		...
		it("Test delete reply by not registered user", async function () {
			...
			await expect(peeranhaContent.connect(signers[10]).deleteReply(signers[10].address, 1, 1))
			.to.be.revertedWith('user_not_found');
		});
	...
	describe('Delete comment', function () {
		...
		it("Test delete comment by moderator", async function () {
			...
			const userRating = changeStartRating(changeStartRating(await peeranhaUser.getUserRating(signers[1].address, 1)));
		...
		it("Test delete comment by not registered user", async function () {
			...
			await expect(peeranhaContent.connect(signers[10]).deleteComment(signers[10].address, 1, 0, 1))
			.to.be.revertedWith('user_not_found');
			await expect(peeranhaContent.connect(signers[10]).deleteComment(signers[10].address, 1, 1, 1))
			.to.be.revertedWith('user_not_found');
		});
	...
	describe('Best reply', function () {		// TODO перенести тесты с лучшим ответом
		it("Test delete expert best reply by moderator", async function () {
			...
			const userRating2 = changeStartRating(await peeranhaUser.getUserRating(signers[1].address, 1));
			...
			const newUserRating = changeStartRating(await peeranhaUser.getUserRating(signers[1].address, 1));
		...
		it("Test delete post after choosing best common reply", async function () { // TODO: post or reply
			await peeranhaContent.createPost(signers[0].address, 1, hashContainer[0], PostTypeEnum.CommonPost, [1], LanguagesEnum.English);
			await peeranhaContent.connect(signers[1]).createReply(signers[1].address, 1, 0, hashContainer[1], false, LanguagesEnum.English);
			const userRating = changeStartRating(await peeranhaUser.getUserRating(signers[1].address, 1));
		...
		it("Test choose best reply after delete expert best reply by moderator", async function () {
			...
			await peeranhaContent.connect(signers[2]).createReply(signers[2].address, 1, 0, hashContainer[1], false, LanguagesEnum.English);
			const userRating = changeStartRating(await peeranhaUser.getUserRating(signers[1].address, 1));
			const userRating2Reply = changeStartRating(await peeranhaUser.getUserRating(signers[2].address, 1));
			...
			const newOldBestReplyRating = changeStartRating(await peeranhaUser.getUserRating(signers[1].address, 1));
			...
			const newAuthorOldBestReplyRating2 = changeStartRating(await peeranhaUser.getUserRating(signers[1].address, 1));
			const newAuthorNewBestReplyRating = changeStartRating(await peeranhaUser.getUserRating(signers[2].address, 1));
		...
		it("Test choose best reply after delete post after choosing best common reply", async function () {
			...
			await peeranhaContent.connect(signers[2]).createReply(signers[2].address, 1, 0, hashContainer[1], false, LanguagesEnum.English);
			const userRating = changeStartRating(changeStartRating(await peeranhaUser.getUserRating(signers[1].address, 1)));
			const userRating2Reply = changeStartRating(changeStartRating(await peeranhaUser.getUserRating(signers[2].address, 1)));
	...
	describe('Test documentation tree', function () {
		...
		it("Test set documentation position for non-existing community", async function () {
			await expect(peeranhaContent.updateDocumentationTree(signers[0].address, 10, hashContainer[0])).to.be.revertedWith('Community does not exist');
		});
